---
title: "WAVE TRANSMISSION IN HUMAN ATHLETIC MOTION"
date: "2026-05-24 06:32:32"
date_vi: "Chủ Nhật 24/05/2026 06:32"
category: "facebook"
...
**📅 Chủ Nhật 24/05/2026 06:32**
<video controls preload="metadata" style="max-width:100%; height:auto;">
  <source src="../media/media/videos/2103247940237386.mp4" type="video/mp4">
...
# 2. Conceptual Foundation: The Wave Body Hypothesis
...
GROUND COMPRESSION
    ↓
ELASTIC REBOUND WAVE
    ↑
UPWARD BODY PROPAGATION
```
...
# 4. Lower Limb: Wave Entry and Modulation Zone
...
| ------- | -------------------- |
| Foot    | Input coupling       |
| Ankle   | Direction filtering  |
| Knee    | Amplitude modulation |
| Hip     | Phase amplification  |

---

# 5. Pelvis: Phase Shift Amplifier
...
# 7. Shoulder Complex: Wave Vector Conversion System
...
# 8. Arm System: Waveguide Structure
...
# 9. Racquet Interface: Impedance Matching Zone
...
# 10. Ball Impact: Wave Termination Event
...
# 12. Fa Jin as Wave Collapse Mechanism
...
# 13. Comparison: Kinetic Chain vs Wave Body

| Feature    | Kinetic Chain | Wave Body Model   |
| ---------- | ------------- | ----------------- |
| Structure  | Segmented     | Continuous field  |
| Timing     | Sequential    | Phase-coordinated |
| Efficiency | Medium        | High (if aligned) |
| Control    | Local joints  | Global system     |

---

# 14. Tennis Forehand as Wave System

FIGURE 1.8 – Forehand Wave Flow

```
GROUND → LEG → PELVIS → CORE → SHOULDER → ARM → RACQUET → BALL
        (continuous wave propagation system)
```

---

# 15. Training Implications
...
# 2. Structural Model: Spiral Body Field
...
```
        ...
        ↻   │   ↺
            │
     ┌────────────┐
     │   DANTIAN   │  ← central wave generator
     └────────────┘
      ↻        ↺
   [LEFT]    [RIGHT]
   ARM/SPINE/LEG FASCIA NETWORK
```

---

# 3. Dantian: Internal Wave Generator
...
# 4. Chan Si Jin (Silk Reeling): Spiral Wave Propagation
...
# 5. Peng Jin: Elastic Field Expansion
...
# 6. Spiral vs Linear Force

| Model       | Linear (Kinetic) | Spiral (Tai Chi Wave) |
| ----------- | ---------------- | --------------------- |
| Path        | Straight         | Helical               |
| Stability   | Low              | High                  |
| Energy loss | Higher           | Lower                 |
| Control     | Segment-based    | Whole-body field      |

---

# 7. Ground Connection: Rooting as Wave Anchoring
...
# 8. Full Body Wave Synchronization
...
```
Leg → Pelvis → Dantian → Spine → Shoulder → Arm
(phase-aligned spiral wave propagation)
```

---

# 9. Fa Jin in Tai Chi Wave Model
...
```
LOAD (spiral compression)
        ↓
ALIGNMENT (phase locking)
        ↓
RESONANCE PEAK
        ↓
WAVE COLLAPSE
...
| Tai Chi          | Tennis                    |
| ---------------- | ------------------------- |
| Dantian rotation | Core coil in forehand     |
| Chan Si Jin      | Racquet lag & whip        |
| Peng Jin field   | Stability in contact zone |
| Fa Jin           | Ball acceleration burst   |

---

# 11. Integrated Wave System (Body + Sport)
...
# 12. Training Implications (Internal Method)
...
# 4. Force Plate (GRF) Wave Structure
...
```
LOW LEVEL PLAYER:
Foot → Knee → Hip → Shoulder → Arm
(uneven timing, gaps)

HIGH LEVEL PLAYER:
|||||||||||||||||||||
(synchronized wave field)
```

---

# 6. Integrated EMG + Force Plate Correlation
...
EMG:   ████ █████ ██████
GRF:      /\/\/\/\/\
         (phase coupled oscillation)
```

---

# 7. Tennis Forehand Experimental Case
...
LEG WAVE PACKET
     ↓
PELVIC AMPLIFICATION
     ↓
CORE RESONANCE
     ↓
SHOULDER PHASE SHIFT
     ↓
ARM WAVEGUIDE
     ↓
RACQUET OUTPUT SPIKE
```

---

# 8. Key Validation Criteria
...
# 11. Neural Timing vs Fascia Wave Timing
...
MOTOR NEURON FIRING
        ↓
FOOT → LEG → HIP → CORE → SHOULDER → ARM
...
```
     ...
     └───────────────┘
            ↓
   =========================
   FASCIA WAVE FIELD SYSTEM
   =========================
     ↓     ↓     ↓     ↓
  LEG → CORE → SHOULDER → ARM
            ↓
         RACQUET
```
...
| --------------- | ----------------- | ------------------------- |
| Control type    | discrete signal   | continuous field          |
| Speed limit     | synaptic delay    | elastic propagation speed |
| Coordination    | sequential        | phase-coherent            |
| Failure mode    | misfire / delay   | wave collapse / desync    |
| Skill signature | mechanical timing | fluid timing              |
...
```
NEURAL ERROR:        STEP BREAK
Foot → HIP → ✖ → ARM

WAVE ERROR:          PHASE DISRUPTION
~~~~~~ ~~~ ~~  X  ~~~~~~
```
...
# 2. Forehand: Horizontal Spiral Wave (Fa Jin Horizontal Expression)
...
GROUND INPUT
     ↓
LEG COMPRESSION WAVE
     ↓
PELVIC ROTATIONAL AMPLIFIER
     ↓
CORE SPIRAL RESONANCE
     ↓
SHOULDER PHASE SHIFT
     ↓
ARM WAVEGUIDE
     ↓
RACQUET RELEASE NODE
     ↓
BALL IMPACT (WAVE TERMINATION)
```
...
# 3. Serve: Vertical Wave Stacking System
...
# 4. Volley: Impedance Matching Wave Reflection
...
| Factor   | Requirement           |
| -------- | --------------------- |
| Wrist    | locked wave conductor |
| Shoulder | damping stabilizer    |
| Core     | anti-rotation anchor  |

---

# 5. Integrated Stroke Comparison

## 5.1 Wave Typology

| Stroke   | Wave Type           | Direction         | Energy Pattern       |
| -------- | ------------------- | ----------------- | -------------------- |
| Forehand | Spiral wave         | horizontal        | continuous release   |
| Serve    | Vertical stack wave | upward + downward | compression collapse |
| Volley   | Reflection wave     | incoming/outgoing | minimal generation   |
...
FOREHAND ← CENTER → VOLLEY
 (spiral)         (reflection)
```

---

# 6. Neural–Fascial–Racket Integration
...
```
Step 1: slow ground load
Step 2: delayed hip rotation
Step 3: core spiral hold
Step 4: late arm release
Step 5: controlled impact
```
...
```
        ...
        NEURAL TRIGGER
               ↓
   =========================
   FASCIA WAVE PROPAGATION
   =========================
        ↓           ↓
   BODY MOTION   IMPACT EVENT
        ↓           ↓
        └──────┬────┘
               ↓
...
| --------------------- | -------------------- | ------------------------- |
| Neural System         | command / initiation | triggers wave start       |
| Fascial System        | transmission medium  | carries wave energy       |
| Proprioceptive System | feedback control     | stabilizes wave coherence |
...
# 13. WAVE SENSATION CHECKLIST (CHECKLIST CẢM NHẬN SÓNG)
...
| Level | Mô tả                               |
| ----- | ----------------------------------- |
| 0     | Rời rạc, dùng tay là chính          |
| 1     | Có cảm nhận đất nhưng chưa liên kết |
| 2     | Có dòng sóng nhưng dễ đứt           |
| 3     | Wave ổn định trong rally chậm       |
| 4     | Wave ổn định dưới áp lực            |
| 5     | Fa Jin state (tự động, không gồng)  |
...
```
        ...
        ┌────────────────┼────────────────┐
        │                │                │
   GROUND LOSS     STRUCTURE BREAK   TIMING ISSUE
        │                │                │
   no rebound      disconnection     early/late
        │                │                │
        └────────────┬───┴───┬────────────┘
                     │       │
             RACKET DECAY  WAVE COLLAPSE
```
...
| ------------- | ----------------- | ------------------- |
| đánh yếu      | mất ground wave   | đứng nhún nhẹ       |
| đánh cứng     | structural break  | thả tay + xoay thân |
| timing gấp    | phase collapse    | chậm lại 50%        |
| mất feel bóng | racket decoupling | swing thả lỏng      |
